...
const header = () => {
  ...
  const toggleNavBtn = createDOMElement(
    "button",
...
const nav = () => {
  const navElement = createDOMElement("nav", { class: "nav" });

  const mainSection = createDOMElement("div", { class: "main-nav" });

  const mainUl = createDOMElement("ul");
  const inboxLi = createDOMElement(
    "li",
    { class: "project-name nav-active" },
    "Inbox"
  );
  ...
  const thisWeekLi = createDOMElement(
    "li",
    { class: "project-name" },
    "This Week"
  );
  ...
  const addProjectBtn = createDOMElement(
    "button",
    { id: "add-project-btn" },
    "Add Project"
  );
  ...
  navElement.appendChild(mainSection);
  navElement.appendChild(projectsSection);

  return navElement;
};

const projectForm = () => {
  const projectFormElement = createDOMElement("form", {
    class: "project-form",
  });
  ...
  projectFormElement.appendChild(input);

  const projectErrorSpan = createDOMElement("span", {
    id: "projectError",
    class: "error",
  });
  projectFormElement.appendChild(projectErrorSpan);

  const addBtn = createDOMElement(
    "button",
    { type: "button", id: "form-add-btn", class: "green-btn" },
    "Add"
  );
  projectFormElement.appendChild(addBtn);

  const cancelBtn = createDOMElement(
    "button",
    { type: "button", id: "form-cancel-btn", class: "red-btn" },
    "Cancel"
  );
  projectFormElement.appendChild(cancelBtn);

  return projectFormElement;
};

function createProjectListItemDiv(projectObj) {
  const projectsListDiv = document.querySelector(".projects-list");
  const projectListItemDiv = createDOMElement("div", {
    class: "projects-list-item",
  });
  const nameContainerDiv = createDOMElement(
    "div",
    { class: "project-name" },
    projectObj.getName()
  );
  const deleteBtnContainerDiv = createDOMElement(
    "div",
    { class: "del-project-btn" },
    "X"
  );
...
const main = () => {
  ...
  const showToDoFormBtn = createDOMElement(
    "button",
    { id: "todo-form-btn" },
    "Add new To-Do"
  );
...
const addNewToDoForm = () => {
  ...
  const titileErrorSpan = createDOMElement("span", {
    id: "titleError",
    class: "error",
  });
  toDoForm.appendChild(titileErrorSpan);

  const descriptionInput = createDOMElement("textarea", {
  ...
  const buttonsSpan = createDOMElement("span", {
    class: "add-cancel-todo-btns",
  });

  const addToDoBtn = createDOMElement(
    "button",
    { type: "button", id: "todo-add-btn", class: "green-btn" },
    "Add"
  );
  buttonsSpan.appendChild(addToDoBtn);

  const cancelToDoBtn = createDOMElement(
    "button",
    { type: "button", id: "todo-cancel-btn", class: "red-btn" },
    "Cancel"
  );
...
function toDoPrioritySelectElement() {
  const prioritySelectElement = createDOMElement("select", {
    id: "priority-input",
  });

  const optionDefault = createDOMElement(
    "option",
    { value: "none" },
    "--Priority"
  );
  ...
  const optionLow = createDOMElement(
    "option",
    { value: "low" },
    "Low Priority"
  );
  ...
  const optionMedium = createDOMElement(
    "option",
    { value: "medium" },
    "Medium Priority"
  );
  ...
  const optionHigh = createDOMElement(
    "option",
    { value: "high" },
    "High Priority"
  );
...
function createToDoCardDiv(toDoObj) {
  ...
  const toDoCardDiv = createDOMElement("div", {
    id: toDoObj.getID(),
    class: `todo-card ${toDoObj.getPriority()}`,
  });
  const toDoMain = createDOMElement("div", { class: "todo-main" });
  const cardTitleDiv = createDOMElement(
    "div",
    { class: "card-title" },
    toDoObj.getTitle()
  );
  toDoMain.append(cardTitleDiv);
  const cardDueDateDiv = createDOMElement(
    "div",
    { class: "card-duedate" },
    toDoObj.getDueDate()
  );
  ...
  const showMore = createDOMElement(
    "button",
  ...
  const toDoDetails = createDOMElement("div", {
    class: "todo-details",
    style: "display: none",
  });
  ...
  const descriptionInfo = createDOMElement(
    "span",
    { class: "description-info" },
    toDoObj.getDescription()
  );
  ...
  const toDoPriority = createDOMElement(
    "span",
    { class: "todo-priority" },
...
export {
  ...
  main,
  projectForm,
  toDoPrioritySelectElement,
  createToDoCardDiv,
  createProjectListItemDiv,
};
